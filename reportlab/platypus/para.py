...
class paragraphEngine:
    ...
    def __init__(self, program=None):
        ...
        self.baseindent = (
            0.0  # adjust this to add more indentation for bullets, eg
        )
    ...
    def fitLine(self, program, totalLength):
        ...
        while not done and cursor < maxcursor:
            ...
            if isinstance(opcode, str) or hasattr(opcode, "width"):
                ...
                if (
                    newlength > maxLength and not first
        ...
        for opcode in program:
            if isinstance(opcode, str) or hasattr(opcode, "width"):
                ...
                if indicator == "nextLine":
                    ...
                    raise ValueError(
                        "don't understand indicator " + repr(indicator)
                    )
            else:
                raise ValueError(
                    "op must be string float or tuple " + repr(opcode)
                )
        laststate = self.__dict__.copy()
        # self.resetState(startstate)
        self.__dict__.update(startstate)
        ...
        if isinstance(parsedText, str):
            ...
            if compiler is not None:
                ...
                if debug:
                    ...
                    raise ValueError(
                        "don't know how to handle tag " + repr(tagname)
                    )
    ...
    def compile_ul(self, attdict, content, extra, program, tagname="ul"):
        ...
        for e in content:
            if isinstance(e, str):
                ...
                if tagname != "li":
                    raise ValueError(
                        "don't expect %s inside list" % repr(tagname)
                    )
                newatts = atts.copy()
                if attdict1:
    ...
    class ulMapper(MapNode):
        ...
        def translate(self, nodetuple, controller, context, overrides):
            ...
            return theParaMapper.translate(
                thepara, controller, context, overrides
            )
