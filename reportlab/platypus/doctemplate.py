        ...
        try:
            ...
            if aerr.args[0] == arn:
                raise NotImplementedError(
                    "Can't handle ActionFlowable(%s)" % action
                )
            else:
                raise
        except:
...
class NotAtTopPageBreak(FrameActionFlowable):
    ...
    def frameAction(self, frame):
        if not frame._atTop:
            frame.add_generated_content(
                PageBreak(nextTemplate=self.nextTemplate)
            )
...
class BaseDocTemplate:
    ...
    def handle_nextPageTemplate(self, pt):
        ...
        if isinstance(pt, strTypes):
            ...
            for ptn in pt:
                ...
                if not found:
                    raise ValueError(
                        "Cannot find page template called %s" % ptn
                    )
            if not c:
    ...
    def _peekNextFrame(self):
        ...
        if hasattr(f, "lastFrame") or f is self.pageTemplate.frames[-1]:
            ...
            return self.pageTemplate.frames[
                self.pageTemplate.frames.index(f) + 1
            ]
    ...
    def handle_keepWithNext(self, flowables):
        ...
        n = len(flowables)
        while (
            i < n and flowables[i].getKeepWithNext() and _ktAllow(flowables[i])
        ):
            i += 1
        if i:
        ...
        if isinstance(f, PageBreak):
            ...
            if frame.add(f, canv, trySplit=self.allowSplitting):
                ...
                if n:
                    if not isinstance(
                        S[0],
                        (PageBreak, SlowPageBreak, ActionFlowable, DDIndenter),
                    ):
                        ...
                        flowables[
                            0:0
                        ] = S  # put split flowables back on the list
                else:
    ...
    def _makeCanvas(self, filename=None, canvasmaker=canvas.Canvas):
        ...
        *NB* only the canvases created in self._startBuild will actually be used
        ...
        if self.displayDocTitle is not None:
            canv.setViewerPreference(
                "DisplayDocTitle", ["false", "true"][self.displayDocTitle]
            )
    ...
    def pageRef(self, label):
        ...
        if verbose:
            print(
                "pageRef called with label '%s' on page %d" % (label, self.page)
            )
