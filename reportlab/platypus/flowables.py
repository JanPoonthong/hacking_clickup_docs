...
class XBox(Flowable):
    ...
    def draw(self):
        ...
        self.canv.drawCentredString(
            0.5 * self.width, 0.5 * self.height, self.text
        )
...
def splitLine(
    ...
    while line_to_split and len(line_to_split) > 0:
        ...
        if first_line:
            ...
            lines_splitted.append(
                new_line_characters + line_to_split[0:split_index]
            )
...
class Preformatted(Flowable):
    ...
    def __init__(
        ...
        if text and maxLineLength:
            self.lines = splitLines(
                self.lines, maxLineLength, splitChars, newLineChars
            )
    ...
    def minWidth(self):
        ...
        return max(
            [stringWidth(line, fontName, fontSize) for line in self.lines]
        )
...
class Image(Flowable):
    ...
    def _restrictSize(self, aW, aH):
        if self.drawWidth > aW + _FUZZ or self.drawHeight > aH + _FUZZ:
            ...
            factor = min(
                float(aW) / self.drawWidth, float(aH) / self.drawHeight
            )
...
def _listWrapOn(
    F, availWidth, canv, mergeSpace=1, obj=None, dims=None, fakeWidth=None
):
...
class PTOContainer(_Container, Flowable):
    ...
    def split(self, availWidth, availHeight):
        ...
        T = R1 + [
            PTOContainer(
                R2, [copy(x) for x in I.trailer], [copy(x) for x in I.header]
            )
        ]
        return T
...
class KeepInFrame(_Container, Flowable):
    ...
    def wrap(self, availWidth, availHeight):
        ...
        if mode == "error" and (W > maxWidth + _FUZZ or H > maxHeight + _FUZZ):
            ...
            self._scale = s1
            if (
                H < min(0.95 * maxHeight, maxHeight - 10)
                or H >= maxHeight + _FUZZ
            ):
...
class ImageAndFlowables(_Container, _FindSplitterMixin, Flowable):
    ...
    def drawOn(self, canv, x, y, _sW=0):
        ...
        if self._C1:
            ...
            _Container.drawOn(
                self, canv, x, y - self._aH, content=self._C1, aW=aW
            )
...
class BalancedColumns(_FindSplitterMixin, NullDraw):
    ...
    def __init__(
        ...
        if nCols < 2:
            raise ValueError(
                "nCols should be at least 2 not %r in %s"
    ...
    def _generated_content(self, aW, aH):
        ...
        lpad = (
            frame._leftPadding
            if self._leftPadding is None
            else self._leftPadding
        )
        rpad = (
            frame._rightPadding
            if self._rightPadding is None
            else self._rightPadding
        )
        tpad = (
            frame._topPadding if self._topPadding is None else self._topPadding
        )
        bpad = (
            frame._bottomPadding
            if self._bottomPadding is None
            else self._bottomPadding
        )
        leftExtraIndent = frame._leftExtraIndent
        rightExtraIndent = frame._rightExtraIndent
        gap = (
            max(lpad, rpad)
            if self._innerPadding is None
            else self._innerPadding
        )
        hgap = gap * 0.5
        canv = self.canv
        nCols = self._nCols
        ...
        showBoundary = (
            self.showBoundary
            if self.showBoundary is not None
            else frame.showBoundary
        )
        ...
        if indenter0:
            G.append(indenter0)
        doBox = (
            self._boxStrokeColor
            and self._boxStrokeWidth
            and self._boxStrokeWidth >= 0
...
class BulletDrawer:
    ...
    def drawOn(self, indenter, canv, x, y, _sW=0):
        ...
        elif bulletAlign.startswith("numeric") or bulletAlign.startswith(
            "decimal"
        ):
...
class ListFlowable(_Container, Flowable):
        ...
        if self._bulletType != "bullet":
            if auto:
                ...
                for v in self._bulletType:
                    if v not in self._numberStyles:
                        raise ValueError(
                            "invalid bullettype=%r" % self._bulletType
                        )
        self._start = start
    ...
    def _makeLIIndenter(self, flowable, bullet, params=None):
        if params:
            ...
            return LIIndenter(
                flowable, self._leftIndent, self._rightIndent, bullet
            )
                ...
                for d, f in self._flowablesIter():
                    if d:
                        ...
                        if b:
                            if b.bulletType == bt:
                                maxW = max(
                                    maxW, _computeBulletWidth(b, b.value)
                                )
                                tvalue = int(b.value)
                        else:
                            maxW = max(maxW, _computeBulletWidth(self, tvalue))
        ...
        for d, f in self._flowablesIter():
            ...
            if d:
                ...
                z = self._makeLIIndenter(
                    f.flowable, bullet=None, params=fparams
                )
                if f.first:
...
class DocAssign(NullDraw):
    ...
    def func(self):
        return self._doctemplateAttr("d" + self.__class__.__name__[1:])(
            *self.args
        )
