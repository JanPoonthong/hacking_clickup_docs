...
def _justifyDrawParaLine(tx, offset, extraspace, words, last=0):
    ...
    simple = (
        ...
        or getattr(tx, "preformatted", False)
    )
    if not simple:
        nSpaces = len(words) + _nbspCount(text) - 1
        simple = not nSpaces
...
def _trailingSpaceLength(text, tx):
    ...
    return (
        tx._canvas.stringWidth(ws.group(), tx._fontname, tx._fontsize)
        if ws
        else 0
    )
            ...
            if isinstance(f, _SplitFrag):
                ...
                if not W:
                    ...
                    if isinstance(lf, _SplitFragHY):
                        sty, t = W[-1]
                        Wlen -= (
                            stringWidth(t[-1], sty.fontName, sty.fontSize)
                            + 1e-8
                        )
                ...
                if W:
                    # must end a joining
                    aR(
                        (_HSFrag if isinstance(lf, _HSFrag) else list)(
                            [W0t(Wlen)] + W
                        )
                    )
                    del W[:]
...
def _splitFragWord(w, maxWidth, maxWidths, lineno):
    ...
    W.append((f, fragText))
    W = (_SplitFragHS if isinstance(w, _HSFrag) else _SplitFragH)(
        [wordWidth] + W
    )
...
def _uri_split_pairs(uri):
    ...
    slash = (
        "\\"
        if not scheme and "/" not in uri  # might be a microsoft pattern
        else "/"
    )
...
def _hyGenPair(
    ...
    if R is not None:
        ...
        if ww > maxWidth or (
            uriWasteReduce and w0 <= (1 - uriWasteReduce) * maxWidth
        ):
    ...
    if hyphenator and (
        _hy_letters_pat.match(s)
        or (_hy_shy_letters_pat.match(s) and "" not in H)
    ):
    ...
    if (
        n >= 3
        and embeddedHyphenation
        and "" not in H
        and _hy_shy_letters_pat.match(s)
    ):
...
def _hyphenateFragWord(
    ...
    if len(FW) == 2:
        ...
        if R:
            ...
            return [
                (_SplitFragHY if jc else _SplitFragH)(
                    [hw + hylen, (f, h + jc)]
                ),
                (_SplitFragHS if isinstance(FW, _HSFrag) else _SplitFrag)(
                    [tw, (f, t)]
                ),
            ]
    else:
...
def _splitWord(
    w, maxWidth, maxWidths, lineno, fontName, fontSize, encoding="utf8"
):
    """
...
def _drawBullet(canvas, offset, cur_y, bulletText, style, rtl):
    ...
    if rtl or style.bulletAnchor != "start":
        ...
        if isStr(bulletText):
            ...
            bulletWidth = stringWidth(
                t, style.bulletFontName, style.bulletFontSize
            )
            if q:
    ...
    if not rtl:
        ...
        width = rtl[0]
        bulletStart = (
            width + style.rightIndent - (style.bulletIndent + bulletWidth)
        )
...
def _handleBulletWidth(bulletText, style, maxWidths):
    ...
    if bulletText:
        ...
        bulletLen = (
            style.bulletIndent + bulletWidth + 0.6 * style.bulletFontSize
        )
...
def _do_under_line(i, x1, ws, tx, us_lines):
    ...
    x2 = x1 + tx._canvas.stringWidth(
        " ".join(tx.XtraState.lines[i][1]), tx._fontname, fs
    )
    for n, k, c, w, o, r, m, g in us_lines:
        ...
        y = (
            ...
            + r
            + _usConv(
                ("-0.125*L" if underline else "0.25*L") if o == "" else o,
                values,
            )
        )
        if not c:
...
def _do_link_line(i, t_off, ws, tx):
    ...
    y = (
        xs.cur_y - i * leading - xs.f.fontSize / 8.0
                    ...
                    for j in xrange(i - 1, limitCheck, -1):
                        ...
                        if uj and category(uj) == "Zs" or ord(uj) >= 0x3000:
                            ...
                            if k < i:
                                j = k + 1
                                extraSpace += sum(
                                    U[ii].width for ii in xrange(j, i)
                                )
                                w = U[k].width
                                u = U[k]
                                i = j
...
class Paragraph(Flowable):
    ...
    def _setup(self, text, style, bulletText, frags, cleaner):
        ...
        self.debug = (
            0  # turn this on to see a pretty one with all the margins etc.
        )
    ...
    def wrap(self, availWidth, availHeight):
        ...
        first_line_width = (
            availWidth
            - (leftIndent + style.firstLineIndent)
            - style.rightIndent
        )
        ...
        if blPara.kind == 1:
            if autoLeading not in ("", "off"):
                ...
                if autoLeading == "max":
                    ...
                    raise ValueError(
                        "invalid autoLeading value %r" % autoLeading
                    )
            else:
    ...
    def split(self, availWidth, availHeight):
        ...
        allowOrphans = getattr(
            self, "allowOrphans", getattr(style, "allowOrphans", 0)
        )
        ...
        n = len(lines)
        allowWidows = getattr(
            self, "allowWidows", getattr(style, "allowWidows", 1)
        )
        if n <= s:
            return [self]
        if not allowWidows:
        ...
        P2 = self.__class__(
            None, style, bulletText=None, frags=func(blPara, s, n)
        )
    ...
    def breakLines(self, width):
        ...
        if attemptHyphenation:
            hymwl = getattr(
                style, "hyphenationMinWordLength", hyphenationMinWordLength
            )
        ...
        if nFrags == 1 and not (
            ...
            while words:
                ...
                if newWidth > maxWidth + spaceShrink and not isinstance(
                    word, _SplitWordH
                ):
                    if attemptHyphenation:
            ...
            while _words:
                ...
                if not words:
                    n = (
                        dSpaceShrink
                    ) = spaceShrink = spaceWidth = currentWidth = 0
                    maxSize = fontSize
                ...
                if (
                    not lineBreak
                    and newWidth > (maxWidth + spaceShrink)
                    ...
                    if splitLongWords and not isinstance(w, _SplitFrag):
                        ...
                        if wordWidth > max(maxWidths[nmw : nmw + 1]):
                            ...
                            continue
                endLine = (
                    newWidth > (maxWidth + spaceShrink) and n > 0
                ) or lineBreak
                if not endLine:
                    if lineBreak:
    ...
    def breakLinesCJK(self, maxWidths):
        ...
        if (
            nFrags == 1
            and not hasattr(frags[0], "cbDefn")
            and not style.endDots
        ):
            f = frags[0]
            ...
            if blPara.kind == 0:
                ...
                if bulletText:
                    offset = _drawBullet(
                        ...
                        bulletText,
                        style,
                        rtl=style.wordWrap == "RTL"
                        and self._wrapWidths
                        or False,
                    )
                ...
                tx.preformatted = (
                    "preformatted" in self.__class__.__name__.lower()
                )
                if autoLeading == "max":
                    leading = max(leading, blPara.ascent - blPara.descent)
                elif autoLeading == "min":
                ...
                tx._underlineOffset = (
                    getattr(style, "underlineOffset", "") or "-0.125f"
                )
...
would follow from the assumption that"""
        P = Paragraph(
            text,
            ParagraphStyle(
                "aaa", parent=styleSheet["Normal"], align=TA_JUSTIFY
            ),
        )
        dumpParagraphFrags(P)
...
It should be a normal<onDraw name="_indexAdd" label="normal"/> paragraph, set in Courier 12 pt."""
        P = Paragraph(
            text,
            style=ParagraphStyle("style1", fontName="Courier", fontSize=10),
        )
        dumpParagraphFrags(P)
