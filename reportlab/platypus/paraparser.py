...
class ParaParser(HTMLParser):
    ...
    def start_super(self, attributes):
        ...
        self._push("super", **A)
        frag = self._stack[-1]
        frag.rise += fontSizeNormalize(
            frag, "supr", frag.fontSize * supFraction
        )
    ...
    def start_sub(self, attributes):
        ...
        frag = self._stack[-1]
        frag.rise -= fontSizeNormalize(
            frag, "supr", frag.fontSize * subFraction
        )
        frag.fontSize = fontSizeNormalize(
            frag, "sups", frag.fontSize - min(sizeDelta, 0.2 * frag.fontSize)
        )
    ...
    def _syntax_error(self, message):
        if (
            message[:10] == "attribute "
            and message[-17:] == " value not quoted"
        ):
            return
        ...
        for k, v in attr.items():
            ...
            if k in attrMap:
                ...
                if func is not None:
                    ...
                    v = (
                        func(self, v)
                        if isinstance(func, _ExValidate)
                        else func(v)
                    )
                A[j[0]] = v
            else:
