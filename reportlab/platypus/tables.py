...
def _hLine(canvLine, scp, ecp, y, hBlocks, FUZZ=rl_config._FUZZ):
    ...
    if (
        not hBlocks
        or scp >= hBlocks[-1][1] - FUZZ
        or ecp <= hBlocks[0][0] + FUZZ
    ):
...
def spanFixDim(V0, V, spanCons, lim=None, FUZZ=rl_config._FUZZ):
    ...
    for v, (x0, x1) in reversed(
        sorted(((iv, ik) for ik, iv in spanCons.items()))
    ):
        if x0 >= lim:
            continue
        x1 += 1
...
class Table(Flowable):
    def __init__(
        ...
        if minRowHeights != None:
            ...
            if not lmrh:
                raise ValueError(
                    "%s Supplied mismatching minimum row heights of length %d"
                    % (self.identity(), lmrh)
                )
            elif lmrh < nrows:
        ...
        if not isinstance(C, _ExpandedCellTuple):
            ...
            for c in self._cellListIter(C, aW, aH):
                if isinstance(c, Indenter):
                    if not frame:
                        frame = CellFrame(
                            _leftExtraIndent=0, _rightExtraIndent=0
                        )
                    c.frameAction(frame)
                    if (
                        frame._leftExtraIndent < 1e-8
                        and frame._rightExtraIndent < 1e-8
                    ):
                        frame = None
                    continue
                if frame:
    ...
    def _elementWidth(self, v, s):
        if isinstance(v, (list, tuple)):
            ...
            if hasattr(v, "width") and isinstance(v.width, (int, float)):
                return v.width
            if hasattr(v, "drawWidth") and isinstance(
        ...
        for c in cmds:
            ...
            if cS:
                ...
                self._addCommand(
                    (c[0],)
                    + (
                        (sc, repeatRows.index(min(cS))),
                        (ec, repeatRows.index(max(cS))),
                    )
                    + tuple(c[3:])
                )
            if er < n:
    ...
    def _splitRows(self, availHeight):
        ...
        if n <= (
            repeatRows if isinstance(repeatRows, int) else (max(repeatRows) + 1)
        ):
            return []
        ...
        if lo:
            ...
            if n > hi:
                return self._splitRows(
                    availHeight - sum(self._rowHeights[hi:n])
                )
            elif n < lo:
                return []
    ...
    def _getFirstPossibleSplitRowPosition(self, availHeight):
        ...
        if self._spanCmds:
            self._getRowImpossible(
                impossible, self._rowSpanCells, self._spanRanges
            )
        if self._nosplitCmds:
            self._getRowImpossible(
    ...
    def split(self, availWidth, availHeight):
        ...
        if self.splitByRow:
            if (
                not rl_config.allowTableBoundsErrors
                and self._width > availWidth
            ):
                return []
        ...
        for cmd, (sc, sr), (ec, er), arg in self._bkgrndcmds:
            ...
            if hasattr(arg, "__call__"):
                ...
                if (
                    ...
                    and arg[0] in ("VERTICAL", "HORIZONTAL")
                ):
                    #
                    ...
                    if direction == "HORIZONTAL":
                        canv.linearGradient(
                            x0, y0, x0 + w, y0, arg, extend=False
                        )
                    else:  # VERTICAL
                        canv.linearGradient(
                            x0, y0, x0, y0 + h, arg, extend=False
                        )
                    canv.restoreState()
                else:
