...
if isPy3:
    ...
    def SUBSETN(
        n,
        table=string.maketrans(b"0123456789", b"ABCDEFGIJK"),
...
class TTFontFile(TTFontParser):
    ...
    def extractInfo(self, charInfo=1):
        ...
        if ver_maj != 1:
            raise TTFError(
                "Unknown head table version %d.%04x" % (ver_maj, ver_min)
            )
        ...
        if "OS/2" in self.table:
            ...
            if fsType == 0x0002 or (fsType & 0x0300):
                subsettingAllowed = (
                    ...
                    not in rl_config.allowTTFSubsetting
                )
        ...
        if ver_maj not in (1, 2, 3, 4):
            ...
            raise TTFError(
                "Unknown post table version %d.%04x" % (ver_maj, ver_min)
            )
        ...
        if ver_maj != 1:
            raise TTFError(
                "Unknown hhea table version %d.%04x" % (ver_maj, ver_min)
            )
        self.skip(28)
        ...
        if ver_maj != 1:
            raise TTFError(
                "Unknown maxp table version %d.%04x" % (ver_maj, ver_min)
            )
            ...
            for n in xrange(segCount):
                for unichar in xrange(startCount[n], endCount[n] + 1):
                    if idRangeOffset[n] == 0:
                        ...
                        offset = (unichar - startCount[n]) * 2 + idRangeOffset[
                            n
                        ]
        ...
        if indexToLocFormat == 0:
            ...
            raise TTFError(
                "Unknown location table format (%d)" % indexToLocFormat
            )
    ...
    def makeSubset(self, subset):
        ...
        hmtx = (
            hmtx[:n] + hmtx[n + 1 :: 2]
        )  # full pairs + all the trailing lsb's
        ...
        for n in xrange(numGlyphs):
            ...
            if glyphLen > 2 and unpack(">h", data[:2])[0] < 0:
                ...
                flags = GF_MORE_COMPONENTS
                while flags & GF_MORE_COMPONENTS:
                    flags = unpack(">H", data[pos_in_glyph : pos_in_glyph + 2])[
                        0
                    ]
                    glyphIdx = unpack(
                        ">H", data[pos_in_glyph + 2 : pos_in_glyph + 4]
                    )[0]
                    data = _set_ushort(
...
class TTFont:
    ...
    def __init__(
        self, name, filename, validate=0, subfontIndex=0, asciiReadable=None
    ):
        """Loads a TrueType font from filename.
        ...
        self.face = TTFontFace(
            filename, validate=validate, subfontIndex=subfontIndex
        )
    ...
    def splitString(self, text, doc, encoding="utf-8"):
        ...
        if cur:
            results.append(
                (curSet, bytes(cur) if isPy3 else "".join(chr(c) for c in cur))
            )
        return results
    ...
    def getSubsetInternalName(self, subset, doc):
        ...
        if state.internalName is None:
            state.internalName = state.namePrefix + repr(
    ...
    def addObjects(self, doc):
        ...
        for n, subset in enumerate(state.subsets):
            ...
            baseFontName = (
                b"".join(
                    (SUBSETN(n), b"+", self.face.name, self.face.subfontNameX)
                )
            ).decode("pdfdoc")
