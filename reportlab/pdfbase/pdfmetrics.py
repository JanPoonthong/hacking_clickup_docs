...
class Encoding:
    ...
    def __init__(self, name, base=None):
        ...
        if name in standardEncodings:
            assert (
                base is None
            ), "Can't have a base encoding for a standard encoding"
    ...
    def isEqual(self, other):
        return self.name == other.name and tuple(self.vector) == tuple(
            other.vector
        )
...
if isPy3:
    ...
    def _pfbSegLen(p, d):
        ...
        return (
            (((ord(d[p + 3]) << 8) | ord(d[p + 2]) << 8) | ord(d[p + 1])) << 8
        ) | ord(d[p])
...
class EmbeddedType1Face(TypeFace):
    ...
    def addObjects(self, doc):
        ...
        fontDescriptorRef = doc.Reference(
            fontDescriptor, "fontDescriptor:" + self.name
        )
...
def registerFontFamily(
    family, normal=None, bold=None, italic=None, boldItalic=None
):
...
def findFontAndRegister(fontName):
    ...
    assert (
        type(fontName) is str
    ), "fontName=%s is not required type str" % ascii(fontName)
