...
def cacheImageFile(filename, returnInMemory=0, IMG=None):
    ...
    cachedname = os.path.splitext(filename)[0] + (
    ...
    if filename == cachedname:
        if cachedImageExists(filename):
            ...
            if returnInMemory:
                return filter(
                    None, open_for_read(cachedname).read().split("\r\n")
                )
        else:
...
class _fusc:
    ...
    def encrypt(self, s):
        return self.__rotate(
            asciiBase85Encode(
                "".join(map(chr, self.__fusc(list(map(ord, s)))))
            ),
            self._n,
        )
    ...
    def decrypt(self, s):
        return "".join(
            map(
                ...
                self.__fusc(
                    list(
                        map(ord, asciiBase85Decode(self.__rotate(s, -self._n)))
                    )
                ),
            )
        )
