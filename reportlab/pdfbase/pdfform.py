...
def buttonFieldAbsolute(
    canvas, title, value, x, y, width=16.7704, height=14.907
):
    ...
    """
    theform = getForm(canvas)
    return theform.buttonField(
        canvas, title, value, x, y, width=width, height=height
    )


def buttonFieldRelative(
    canvas, title, value, xR, yR, width=16.7704, height=14.907
):
    "same as buttonFieldAbsolute except the x and y are relative to the canvas coordinate transform"
    (xA, yA) = canvas.absolutePosition(xR, yR)
    return buttonFieldAbsolute(
        canvas, title, value, xA, yA, width=width, height=height
    )
...
def selectFieldAbsolute(canvas, title, value, options, x, y, width, height):
    ...
    theform = getForm(canvas)
    theform.selectField(
        canvas, title, value, options, x, y, x + width, y + height
    )
...
def selectFieldRelative(canvas, title, value, options, xR, yR, width, height):
    ...
    return selectFieldAbsolute(
        canvas, title, value, options, xA, yA, width, height
    )
...
class AcroForm(PDFObject):
    ...
    def buttonField(
        ...
        page = doc.thisPageRef()
        field = ButtonField(
            title, value, xmin, ymin, page, width=width, height=height
        )
        self.fields.append(field)
        canvas._addAnnotation(field)
...
def ButtonStream(content, width=16.7704, height=14.907):
    result = PDFStream(
        buttonStreamDictionary(width=width, height=height), content
    )
    result.filters = []
    return result
