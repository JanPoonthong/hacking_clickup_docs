...
class PDFDocument(PDFObject):
    ...
    def SaveToFile(self, filename, canvas):
        if getattr(self, "_savedToFile", False):
            raise RuntimeError(
                "class %s instances can only be saved once"
    ...
    def inPage(self):
        ...
        if self.inObject is not None:
            if self.inObject == "page":
                return
            raise ValueError(
                "can't go in page already in object %s" % self.inObject
            )
        self.inObject = "page"
    ...
    def addColor(self, cmyk):
        ...
        if not sname:
            ...
            if not sname:
                raise ValueError(
                    "CMYK colour %r used without a spotName" % cmyk
                )
            else:
    ...
    def getFormBBox(self, name, boxType="MediaBox"):
        ...
        if internalname in self.idToObject:
            ...
            if isinstance(theform, PDFFormXObject):
                ...
                raise ValueError(
                    "I don't understand the form instance %s" % repr(name)
                )
...
class PDFString(PDFObject):
    ...
    def format(self, document):
        ...
        if isBytes(s):
            ...
            raise ValueError(
                "PDFString argument must be str/unicode not %s" % type(s)
            )
...
class PDFDictionary(PDFObject):
    ...
    def format(self, document, IND=b"\n "):
        ...
        if (
            self.multiline and rl_config.pdfMultiLine
        ) or self.multiline == "forced":
            L = IND.join(L)
        else:
...
class PDFObjectReference(PDFObject):
    ...
    def format(self, document):
        try:
            return pdfdocEnc(
                "%s %s R" % document.idToObjectNumberAndVersion[self.name]
            )
        except:
            raise KeyError(
...
class PDFPageLabel(PDFCatalog):
    ...
    __convertible__ = (
        "ARABIC ROMAN_UPPER ROMAN_LOWER LETTERS_UPPER LETTERS_LOWER"
    )
...
class PDFOutlines(PDFObject):
    ...
    def addOutlineEntry(
        self, destinationname, level=0, title=None, closed=None
    ):
...
class HighlightAnnotation(Annotation):
    ...
    def __init__(
        self, Rect, Contents, QuadPoints, Color=[0.83, 0.89, 0.95], **kw
    ):
        self.Rect = Rect
        self.Contents = Contents
        self.otherkw = kw
...
class PDFDate(PDFObject):
    ...
    def format(self, doc):
        dfmt = self.dateFormatter or (
            lambda yyyy, mm, dd, hh, m, s: "D:%04d%02d%02d%02d%02d%02d+00'00'"
            % (yyyy, mm, dd, hh, m, s)
        )
...
class PDFDestinationXYZ(PDFObject):
    ...
    def format(self, document):
        ...
        A = PDFArray(
            [pageref, PDFName(self.typename), self.left, self.top, self.zoom]
        )
        return format(A, document)
...
class PDFImageXObject(PDFObject):
    ...
    def __init__(self, name, source=None, mask=None):
        ...
        if source is None:
            ...
            try:
                if not (
                    ext in (".jpg", ".jpeg") and self.loadImageFromJPEG(src)
                ):
                    if rl_config.useA85:
...
class PDFSeparationCMYKColor:
    def __init__(self, cmyk):
        ...
        if not isinstance(cmyk, CMYKColor):
            raise ValueError(
                "%s needs a CMYKColor argument" % self.__class__.__name__
            )
