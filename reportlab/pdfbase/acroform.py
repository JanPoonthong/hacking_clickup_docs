...
class RadioGroup(PDFObject):
    def __init__(
        self, name, tooltip="", fieldFlags="noToggleToOff required radio"
    ):
        if not name:
    ...
    def format(self, doc):
        ...
        if d < 2:
            raise ValueError(
                "RadioGroup:%s has %d < 2 RadioBoxes" % (self.T, d)
            )
...
class AcroForm(PDFObject):
    ...
    def canv(self):
        ...
        if _canv is None:
            raise ValueError(
                "%s.canv is no longer available" % self.__class__.__name__
            )
        return _canv
    ...
    def makeFont(self, fontName):
        ...
        if fontName not in self.formFontNames:
            raise ValueError(
                "form font name, %r, is not one of the standard 14 fonts"
                % fontName
            )
    ...
    def _textfield(
        ...
        if wkind != "textfield":
            ...
            if not isinstance(options, (list, tuple)):
                raise TypeError(
                    "%s options=%r is wrong type" % (wkind, options)
                )
            for v in options:
                if isStr(v):
