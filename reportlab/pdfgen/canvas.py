...
def _normalizeColors(colors):
    ...
    for aColor in colors:
        ...
        if space is not None and space != nspace:
            raise ValueError(
                "Mismatch in color spaces: %s and %s" % (space, nspace)
            )
        space = nspace
...
class Canvas(_PDFColorSetter):
    ...
    def __init__(
        ...
        self._initialFontName = (
            initialFontName
            if initialFontName
            else rl_config.canvas_basefontname
        )
        self._initialFontSize = (
            initialFontSize if initialFontSize is not None else 12
        )
    ...
    def init_graphics_state(self):
        ...
        ) = rl_config.canvas_baseColor or (0, 0, 0)
    ...
    def _bookmarkReference(self, name):
        ...
        try:
            ...
            result = d[name] = pdfdoc.Destination(
                name
    ...
    def arc(self, x1, y1, x2, y2, startAng=0, extent=90):
        ...
        pathobject.PDFPathObject(code=self._code).arc(
            x1, y1, x2, y2, startAng, extent
        )
    ...
    def ellipse(self, x1, y1, x2, y2, stroke=1, fill=0):
        ...
        """
        pathobject.PDFPathObject(code=self._code).ellipse(
            x1, y1, x2 - x1, y2 - y1
        )
    ...
    def roundRect(self, x, y, width, height, radius, stroke=1, fill=0):
        ...
        pathobject.PDFPathObject(code=self._code).roundRect(
    ...
    def linearGradient(
        self, x0, y0, x1, y1, colors, positions=None, extend=True
    ):
    ...
    def setLineJoin(self, mode):
        ...
        assert mode in (
            ...
            1,
            2,
        ), "Line Joins allowed: 0=mitre, 1=round, 2=bevel"
        self._lineJoin = mode
        self._code.append("%d j" % mode)
    ...
    def _strokeAndFill(self, stroke, fill, fillMode=None):
        self._code.append(
            PATH_OPS[
                stroke,
                fill,
                fillMode if fillMode is not None else self._fillMode,
            ]
        )
