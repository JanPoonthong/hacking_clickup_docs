...
class PDFTextObject(_PDFColorSetter):
    ...
    def setTextRenderMode(self, mode):
        ...
        assert mode in (
            ...
            6,
            7,
        ), "mode must be in (0,1,2,3,4,5,6,7)"
    ...
    def _formatText(self, text):
        ...
        if log2vis and self.direction in ("LTR", "RTL"):
            ...
            text = log2vis(
                text, directionsMap.get(self.direction, DIR_ON), clean=True
            )
        canv = self._canvas
        font = pdfmetrics.getFont(self._fontname)
        R = []
    ...
    def _textOut(self, text, TStar=0):
        ...
        self._code.append(
            "%s%s" % (self._formatText(text), (TStar and " T*" or ""))
        )
    ...
    def textLines(self, stuff, trim=1):
        ...
        if isStr(stuff):
            ...
            assert (
                1 == 0
            ), "argument to textlines must be string,, list or tuple"
