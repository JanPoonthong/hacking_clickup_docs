...
class Ean13BarcodeWidget(PlotArea):
    ...
    def __init__(self, value="123456789012", **kw):
        ...
        self.value = (
            max(self._digits - len(value), 0) * "0" + value[: self._digits]
        )
        for k, v in kw.items():
            setattr(self, k, v)
...
class Ean5BarcodeWidget(Ean13BarcodeWidget):
    ...
    _attrMap = AttrMap(
        BASE=Ean13BarcodeWidget,
        price=AttrMapValue(
            isBoolean, desc="whether to display the price or not"
        ),
...
class ISBNBarcodeWidget(Ean13BarcodeWidget):
    ...
    _attrMap = AttrMap(
        BASE=Ean13BarcodeWidget,
        price=AttrMapValue(
            NoneOr(nDigits(5)), desc="None or the price to display"
        ),
    )
