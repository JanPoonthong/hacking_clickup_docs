...
class USPS_4State(Barcode):
    ...
    pitch = property(
        lambda self: self.scale("pitch", self.dimensions, self.widthScale)
    )
    barWidth = property(
        lambda self: self.scale("width", self.dimensions, self.widthScale)
    )
    barHeight = property(
        lambda self: self.scale("F", self.tops, self.heightScale)
    ...
    def value(self):
        ...
        return " ".join(
            (tracking[0:2], tracking[2:5], tracking[5:11], tracking[11:])
            + routing
        )
