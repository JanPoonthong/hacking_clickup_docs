...
class _PDFRenderer(Renderer):
    ...
    def drawImage(self, image):
        ...
        if path and (hasattr(path, "mode") or os.path.exists(image.path)):
            self._canvas.drawInlineImage(
                path, image.x, image.y, image.width, image.height
            )
    ...
    def drawPolyLine(self, polyline):
        if self._stroke:
            ...
            ), "Polyline must have 2 or more points"
            head, tail = (
                polyline.points[0:2],
                polyline.points[2:],
            )
    ...
    def drawEllipse(self, ellipse):
        ...
        self._canvas.ellipse(
            x1, y1, x2, y2, fill=self._fill, stroke=self._stroke
        )
        ...
        if self._fill:
            ...
            if not text_anchor in ["start", "inherited"]:
                ...
                if text_anchor == "end":
                    ...
                    x -= numericXShift(
                        text_anchor, text, textLen, font, font_size, enc
                    )
                else:
                    raise ValueError(
                        "bad value for textAnchor " + str(text_anchor)
                    )
    ...
    def drawPath(self, path):
        ...
        fill = self._fill
        stroke = self._stroke
        isClosed = _renderPath(
            path, drawFuncs, forceClose=fill and autoclose == "pdf"
        )
...
def drawToFile(
    ...
    for x in ("Name", "Size"):
        a = "initialFont" + x
        canvasKwds[a] = getattr(
            d, a, canvasKwds.pop(a, STATE_DEFAULTS["font" + x])
        )
...
def drawToString(
    d, msg="", showBoundary=rl_config._unset_, autoSize=1, canvasKwds={}
):
