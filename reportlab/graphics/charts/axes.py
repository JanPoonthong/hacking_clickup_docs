    ...
    def __call__(self, axis):
        ...
        try:
            ...
            func = axis._getLineFunc(
                s - offs, e - offs, kwds.pop("parent", None)
            )
            if not hasattr(axis, "_tickValues"):
                axis._pseudo_configure()
...
class _AxisG(Widget):
    ...
    def _getLineFunc(self, start, end, parent=None):
        ...
        if _3d_dx is not None:
            ...
            return (
                lambda v, s=start, e=end, f=f, _3d_dx=_3d_dx, _3d_dy=_3d_dy: f(
...
class CategoryAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        visibleTicks=AttrMapValue(
            isBoolean, desc="Display axis ticks, if true."
        ),
        visibleLabels=AttrMapValue(
            isBoolean, desc="Display axis labels, if true."
        ),
        visibleGrid=AttrMapValue(isBoolean, desc="Display axis grid, if true."),
        strokeWidth=AttrMapValue(
            isNumber, desc="Width of axis line and ticks."
        ),
        strokeColor=AttrMapValue(
            isColorOrNone, desc="Color of axis line and ticks."
        ),
        ...
        loPad=AttrMapValue(
            isNumber, desc="extra inner space before start of the axis"
        ),
        hiPad=AttrMapValue(
            isNumber, desc="extra inner space after end of the axis"
        ),
...
def _assertXAxis(axis):
    assert axis.isXAxis, (
        "Cannot connect to other axes (%s), but X- ones."
        % axis.__class__.__name__
    )
...
class ValueAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        forceZero=AttrMapValue(
            EitherOr((isBoolean, OneOf("near"))),
            desc="Ensure zero in range if true.",
        ),
        visible=AttrMapValue(isBoolean, desc="Display entire object, if true."),
        visibleAxis=AttrMapValue(isBoolean, desc="Display axis line, if true."),
        visibleLabels=AttrMapValue(
        ...
        avoidBoundFrac=AttrMapValue(
            EitherOr(
                (isNumberOrNone, SequenceOf(isNumber, emptyOK=0, lo=2, hi=2))
            ),
        ...
        tickAxisMode=AttrMapValue(
            OneOf("high", "low", "axis"),
            desc="Like joinAxisMode, but for the ticks",
        ),
        reverseDirection=AttrMapValue(
            isBoolean, desc="If true reverse category direction."
        ),
    ...
    def _setRange(self, dataSeries):
        ...
        if origShiftMin is not None or origShiftIPC is not None:
            ...
            if (
                ...
                if origShiftMin:
                    m = max(
                        m, (valueMax - valueMin) * origShiftMin / self._length
                    )
                self._valueMin -= m
    ...
    def scale(self, value):
        ...
        assert (
            self._configured
        ), "Axis cannot scale numbers before it is configured"
        if value is None:
            value = 0
    ...
    def _xAxisTicker(self, xVals):
        ...
        if self.specifiedTickDates:
            ...
            if (
                ...
                and (axisLength / float(ticks[-1] - ticks[0]))
                * (ticks[1] - ticks[0])
                <= W
            ):
        ...
        def addTick(
            i, xVals=xVals, formatter=formatter, ticks=ticks, labels=labels
        ):
        ...
        for d in _NDINTM:
            ...
            if k <= maximumTicks and k * W <= axisLength:
                ...
                return ticks, labels
        raise ValueError(
            "Problem selecting NormalDate value axis tick positions"
        )
...
class LogValueAxis(ValueAxis):
    ...
    def _setRange(self, dataSeries):
        ...
        if valueMin <= 0:
            raise ValueError(
                "%s: valueMin=%r negative values are not allowed!" % valueMin
            )
        abS = self.avoidBoundSpace
        if abS:
