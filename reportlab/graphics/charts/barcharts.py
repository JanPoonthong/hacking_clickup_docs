...
class BarChartProperties(PropHolder):
    _attrMap = AttrMap(
        strokeColor=AttrMapValue(
            isColorOrNone, desc="Color of the bar border."
        ),
        fillColor=AttrMapValue(
            isColorOrNone, desc="Color of the bar interior area."
        ),
        ...
        swatchMarker=AttrMapValue(
            NoneOr(isSymbol),
            desc="None or makeMarker('Diamond') ...",
            advancedUsage=1,
        ),
...
class BarChart(PlotArea):
    ...
    _attrMap = AttrMap(
        ...
        groupSpacing=AttrMapValue(
            isNumber, desc="Width between groups of bars."
        ),
        barSpacing=AttrMapValue(
            isNumber, desc="Width between individual bars."
        ),
    ...
    annotations=7""",
        ...
        seriesOrder=AttrMapValue(
            SequenceOf(
                SequenceOf(isInt, emptyOK=0, NoneOK=0, lo=1),
                emptyOK=0,
                NoneOK=1,
                lo=1,
            ),
    ...
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        fillColor = getattr(
            style, "fillColor", getattr(baseStyle, "fillColor", None)
        )
                ...
                try:
                    ...
                    raise ValueError(
                        ...
                        % (
                            ...
                            zIndex,
                            "\n".join(
                                ["%s=%r" % (k, Z[k]) for k in sorted(Z.keys())]
                            ),
                        )
                    )
                if k not in Z:
            ...
            if datum is None:
                ...
                if -1e-8 < height <= 1e-8:
                    ...
                    if datum < -1e-8:
                        height = -1e-8
            barRow.append(
                flipXY and (y, x, height, width) or (x, y, width, height)
            )

        if style != "mixed":
            for rowNo, row in enumerate(
                data
            ):  # iterate over the separate series
                barRow = []
    ...
    def _addLabel(
        ...
        if label.visible:
            ...
            if sC and sW:
                g.insert(
                    0, Line(x00, y00, x0, y0, strokeColor=sC, strokeWidth=sW)
                )
            g.add(label)
    ...
    def _makeBars(self, g, lg):
        ...
        for rowNo, row in enumerate(BP):
            ...
            for colNo, (x, y, width, height) in enumerate(row):
                style = (
                    (styleIdx, colNo) in bars
                    and bars[(styleIdx, colNo)]
                    or rowStyle
                )
                if None in (width, height):
                    if not catNAL or colNo in catNNA:
                        self._addNABarLabel(
                            lg, rowNo, colNo, x, y, width, height
                        )
                ...
                if symbol:
                    ...
                    and abs(height) >= 1e-7
                    and (
                        style.fillColor is not None
                        or style.strokeColor is not None
                    )
                ):
    ...
    def _computeLabelPosition(
        self, text, label, rowNo, colNo, x, y, width, height
    ):
        if label.visible:
            labelWidth = stringWidth(text, label.fontName, label.fontSize)
            flipXY = self._flipXY
    ...
    def _computeBarPositions(self):
        ...
        if self._flipXY:
            cA.setPosition(
                self._drawBegin(self.x, self.width), self.y, self.height
            )
        else:
            cA.setPosition(
                self.x, self._drawBegin(self.y, self.height), self.width
            )
        cA.configure(self._configureData)
        self.calcBarPositions()
    ...
    def makeBars(self):
        ...
        if cAStyle == "stacked":
            ...
            fg_value = [
                _[1]
                for _ in sorted(
                    (((t[1], t[2], t[3], t[4]), t) for t in fg_value)
                )
            ]
