...
class WedgeLabel(Label):
    ...
    def _getBoxAnchor(self):
        ...
        if ba in ("autox", "autoy"):
            ...
            ba = (
                getattr(self, "_anti", None)
                and _ANGLE2RBOXANCHOR
                or _ANGLE2BOXANCHOR
            )[na]
            self._checkDXY(ba)
        return ba
...
def _addWedgeLabel(
    self, text, angle, labelX, labelY, wedgeStyle, labelClass=WedgeLabel
):
    # now draw a label
...
class Pie(AbstractPieChart):
    ...
    def makePointerLabels(self, angles, plMode):
        class PL:
            def __init__(
                self, centerx, centery, xradius, yradius, data, lu=0, ru=0
            ):
        ...
        for i, A in angles:
            ...
            n += 1
            l = _addWedgeLabel(
                self, sn, 180, labelX, labelY, style, labelClass=WedgeLabel
            )
            L_add(l)
            b = l.getBounds()
            w = b[2] - b[0]
            h = b[3] - b[1]
            ri = [
                (a[0], intervalIntersection(A, (a[1], a[2]))) for a in refArcs
            ]
...
class Pie3d(Pie):
    ...
    def draw(self):
        ...
        for i in range(n):
            ...
            strokeColor = (
                _getShaded(
                    style.strokeColor, style.strokeColorShaded, style.shading
                )
                or fillColor
            )
            ...
            if depth_3d:
                ...
                p.addArc(
                    cx1, cy1, radiusx, angle0, angle1, yradius=radiusy, moveTo=1
                )
