            ...
            if uSymbol:
                ...
                for j, xy in enumerate(row):
                    if (styleRowNo, j) in lines:
                        juSymbol = getattr(
                            lines[styleRowNo, j], "symbol", uSymbol
                        )
                    else:
                        juSymbol = uSymbol
                    ...
                    if symbol:
                        if bubblePlot:
                            symbol.size = (
                                bubbleR * (drow[j][2] / bubbleMax) ** 0.5
                            )
                        g.add(symbol)
            else:
                if bubblePlot:
...
class GridLinePlot(SimpleTimeSeriesPlot):
    ...
    def demo(self, drawing=None):
        ...
        c0 = colors.PCMYKColor(
            100, 65, 0, 30, spotName="PANTONE 288 CV", density=100
        )
        lp.lines[0].strokeColor = c0
        lp.lines[0].strokeWidth = 2
        lp.lines[0].strokeDashArray = None
        c1 = colors.PCMYKColor(
            0, 79, 91, 0, spotName="PANTONE Wm Red CV", density=100
        )
...
def _maxWidth(T, fontName, fontSize):
    ...
    return (
        ...
        and max(
            list(
                map(
                    lambda t, sW=stringWidth, fN=fontName, fS=fontSize: sW(
                        t, fN, fS
                    ),
                    T,
                )
