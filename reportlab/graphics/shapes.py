...
class Group(Shape):
    ...
    _attrMap = AttrMap(
        ...
        strokeOverprint=AttrMapValue(
            isBoolean, desc="Turn on stroke overprinting"
        ),
    ...
    def expandUserNodes(self):
        ...
        obj = (
            isinstance(self, Drawing)
            and Drawing(self.width, self.height)
            or Group()
        )
...
class Drawing(Group, Flowable):
    ...
    _xtraAttrMap = AttrMap(
        ...
        initialFontName=AttrMapValue(
            isStringOrNone,
            desc="override the STATE_DEFAULTS value for fontName",
        ),
        initialFontSize=AttrMapValue(
            isNumberOrNone,
            desc="override the STATE_DEFAULTS value for fontSize",
        ),
    )
    ...
    def save(
        ...
        **kw
        ...
        plotMode = [
            x.lower() for x in (formats or getattr(self, "formats", ["pdf"]))
        ]
        verbose = (
            verbose is not None
            and (verbose,)
            or (getattr(self, "verbose", verbose),)
        )[0]
        ...
        if "pdf" in plotMode:
            ...
            renderPDF.drawToFile(
                ...
                showBoundary=getattr(
                    self, "showBorder", rl_config.showBoundary
                ),
    ...
    def resized(self, kind="fit", lpad=0, rpad=0, bpad=0, tpad=0):
        ...
        if kind == "fit" or (
            ...
            kind == "expandx" and (xL < lpad or xH > oW - rpad)
        ):
            drawing.width = xH - xL + lpad + rpad
            drawing.transform = (1, 0, 0, 1, lpad - xL, 0)
        elif kind == "fity" or (
            kind == "expandy" and (yL < bpad or yH > oH - tpad)
        ):
            drawing.height = yH - yL + tpad + bpad
...
class Image(SolidShape):
    ...
    _attrMap = AttrMap(
        ...
        y=AttrMapValue(isNumber),
        width=AttrMapValue(
            isNumberOrNone, desc="width of the object in points"
        ),
        height=AttrMapValue(
            isNumberOrNone, desc="height of the objects in points"
        ),
