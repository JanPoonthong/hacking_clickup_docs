...
class PSCanvas:
    ...
    def drawImage(
        ...
        if self.PostScriptLevel == 1:
            ...
            raise ValueError(
                "Unsupported Postscript Level %s" % self.PostScriptLevel
            )
    ...
    def _t1_re_encode(self):
        ...
        for fontName in self._fontsUsed:
            ...
            if (
                not fontObj._dynamicFont
                and fontObj.encName == "WinAnsiEncoding"
            ):
...
"""
    ...
    def line(self, x1, y1, x2, y2):
        if self._strokeColor != None:
            ...
            self.code_append(
                "%s m %s l stroke" % (fp_str(x1, y1), fp_str(x2, y2))
            )
    ...
    def drawCentredString(self, x, y, text, text_anchor="middle"):
        if self._fillColor is not None:
            ...
            if text_anchor == "end":
                ...
                x -= numericXShift(
                    text_anchor, text, textLen, self._font, self._fontSize
                )
    ...
    def _fillAndStroke(self, code, clip=0, fill=1, stroke=1, fillMode=None):
        ...
        if fill or stroke or clip:
            ...
            if fill:
                ...
                self.setColor(self._fillColor)
                self.code_append(
                    "eofill" if fillMode == FILL_EVEN_ODD else "fill"
                )
                if stroke or clip:
                    self.code_append("grestore")
            if stroke:
    ...
    def _drawImageLevel1(self, image, x1, y1, width=None, height=None):
        ...
        self.code.extend(
            [
                "gsave",
                "%s %s translate"
                % (x1, y1),  # need to start are lower left of image
        ...
        self.code.extend(
            [
                ...
                "[%s %s %s %s %s %s]"
                % (imgwidth, 0, 0, -imgheight, 0, imgheight),
...
/BitsPerComponent %d"""
        ...
        self.code.extend(
            [
                "/ImageMatrix [%s 0 0 %s 0 %s]"
                % (imwidth, -imheight, imheight),
...
def draw(drawing, canvas, x=0, y=0, showBoundary=rl_config.showBoundary):
    ...
    R = _PSRenderer()
    R.draw(
        renderScaledDrawing(drawing), canvas, x, y, showBoundary=showBoundary
    )
...
class _PSRenderer(Renderer):
        ...
        if self._canvas._fillColor:
            ...
            if not text_anchor in ["start", "inherited"]:
                ...
                if text_anchor == "end":
                    ...
                    raise ValueError(
                        "bad value for text_anchor " + str(text_anchor)
                    )
    ...
    def drawImage(self, image):
        ...
        self._canvas.drawImage(
            im._image, image.x, image.y, image.width, image.height
        )
