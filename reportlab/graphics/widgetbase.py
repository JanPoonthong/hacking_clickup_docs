...
class ScaleWidget(Widget):
    ...
    def draw(self):
        return shapes.Group(
            transform=(self.scale, 0, 0, self.scale, self.x, self.y),
            *self.contents
        )
...
class Face(Widget):
    ...
    def draw(self):
        ...
        g.add(
            shapes.Circle(s * 0.35, s * 0.65, s * 0.05, fillColor=self.eyeColor)
        )
        ...
        g.add(
            shapes.Circle(s * 0.65, s * 0.65, s * 0.05, fillColor=self.eyeColor)
        )
...
class CandleStickProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        boxStrokeWidth=AttrMapValue(
            NotSetOr(isNumber), desc="Width of the box lines."
        ),
        boxLo=AttrMapValue(
    ...
    def __init__(self, **kwds):
        ...
        self.boxStrokeDashArray = kwds.pop(
            "boxStrokeDashArray", NotSetOr._not_set
        )
    ...
    def __call__(self, _x, _y, _size, _color):
        ...
        boxStrokeColor = NotSetOr.conditionalValue(
            self.boxStrokeColor, strokeColor
        )
        boxStrokeWidth = NotSetOr.conditionalValue(
            self.boxStrokeWidth, strokeWidth
        )
