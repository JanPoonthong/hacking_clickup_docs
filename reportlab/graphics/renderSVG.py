...
TEXT_STYLES = (
    "font-family font-weight font-style font-variant font-size id".split()
)
...
class SVGCanvas:
    def __init__(
        self, size=(300, 300), encoding="utf-8", verbose=0, bom=False, **kwds
    ):
        """
    ...
    def roundRect(
        self, x1, y1, x2, y2, rx=8, ry=8, link_info=None, **_svgAttrs
    ):
    ...
    def drawRightString(self, text, x, y, angle=0, link_info=None):
        self.drawCentredString(
            text, x, y, angle=angle, text_anchor="end", link_info=link_info
        )
        ...
        if self._canvas._fillColor:
            ...
            if not text_anchor in ("start", "inherited"):
                ...
                if text_anchor == "end":
                    ...
                    x -= numericXShift(
                        text_anchor, text, textLen, font, fontSize
                    )
                else:
                    raise ValueError(
                        "bad value for text_anchor " + str(text_anchor)
