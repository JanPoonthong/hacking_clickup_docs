...
class ShadedRect(Widget):
    ...
    _attrMap = AttrMap(
        ...
        numShades=AttrMapValue(
            isNumber, desc="The number of interpolating colors."
        ),
        fillColorStart=AttrMapValue(
            isColorOrNone, desc="Start value of the color shade."
        ),
        fillColorEnd=AttrMapValue(
            isColorOrNone, desc="End value of the color shade."
        ),
        strokeColor=AttrMapValue(
            isColorOrNone, desc="Color used for border line."
        ),
        strokeWidth=AttrMapValue(isNumber, desc="Width used for lines."),
    ...
    def draw(self):
        ...
        for v in V:
            stripe = (
                vertical and Rect(v, y, w / num, h) or Rect(x, v, w, h / num)
            )
            if self.cylinderMode:
...
class ShadedPolygon(Widget, LineShape):
    ...
    def draw(self):
        ...
        if angle == 180:
            ...
            angle = 0
        rect = ShadedRect(
            strokeWidth=0, strokeColor=None, orientation=orientation
        )
