...
class EventCalendar(Widget):
    ...
    def getRelevantTalks(self, talkList):
        ...
        for talk in talkList:
            ...
            assert (
                trackId != 0
            ), "trackId must be None or 1,2,3... zero not allowed!"
            if day == self.day:
                if (
                    (self.startTime is None)
                    or ((hours + duration) >= self.startTime)
    ...
    def scaleTime(self, theTime):
        ...
        proportionUp = (theTime - self._startTime) / (
