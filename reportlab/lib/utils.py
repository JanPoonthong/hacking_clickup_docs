...
from base64 import (
    decodebytes as base64_decodestring,
    encodebytes as base64_encodestring,
)
...
if isPy3:
    ...
    def isNonPrimitiveInstance(x):
        return isinstance(x, types.InstanceType) or not isinstance(
            x,
            (
                float,
                int,
    ...
    from string import (
        letters as ascii_letters,
        uppercase as ascii_uppercase,
        lowercase as ascii_lowercase,
    )
...
try:
    ...
    if not zipImported(ldr=__rl_loader__):
        raise NotImplementedError(
            "can't handle compact distro type %r" % __rl_loader__
        )
...
def open_for_read(name, mode="b", urlopen=urlopen, datareader=datareader):
    ...
    try:
        ...
        try:
            return getBytesIO(
                datareader(name)
                if name[:5].lower() == "data:"
                else urlopen(name).read()
            )
        except:
...
def rl_isdir(
    ...
    return (
        len(
            list(
                filter(
                    lambda x, pn=pn: x.startswith(pn),
                    list(__rl_loader__._files.keys()),
                )
            )
        )
...
def rl_listdir(
    ...
    if not pn.endswith(os.sep):
        pn += os.sep
    return [
        x[len(pn) :] for x in __rl_loader__._files.keys() if x.startswith(pn)
    ]
        ...
        if _isPILImage(fileName):
            ...
            try:
                ...
                if isinstance(self.fp, _bytesIOType):
                    imageReaderFlags = (
                        0  # avoid messing with already internal files
                    )
        ...
        if sys.platform[0:4] == "java":
            ...
            if "transparency" in self._image.info:
                ...
                if isPy3:
                    ...
                    return [
                        ord(c) for c in palette[transparency : transparency + 3]
                    ]
            else:
                return None
        ...
        **kw
        ...
        for n, m in sys.modules.items():
            if (
                ...
                v = [
                    getattr(m, x, None)
                    for x in ("__version__", "__path__", "__file__")
                ]
    ...
    def _show_module_versions(self, k, v):
        ...
        for k in K:
            ...
            try:
                ...
                m = sys.modules[k]
                d = (
                    getattr(m, "__version__", None) == vk0
                    and "SAME"
                    or "DIFFERENT"
                )
            except:
...
def _raccess_get_nodes(a):
    ...
    return reversed(
        [n for n in ast.walk(N[0]) if isinstance(n, _raccess_ast_types)]
    )
