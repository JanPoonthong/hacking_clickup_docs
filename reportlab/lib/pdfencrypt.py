...
class StandardEncryption:
    ...
    def __init__(
        ...
        self.O = (
            self.U
        ) = self.P = self.key = self.OE = self.UE = self.Perms = None
    ...
    def encode(self, t):
        ...
        return encodePDF(
            self.key, self.objnum, self.version, t, revision=self.revision
        )
    ...
    def prepare(self, document, overrideID=None):
        ...
        if DEBUG:
            print(
                "StandardEncryption.prepare(...) - revision %d" % self.revision
            )
        if self.prepared:
        ...
        if self.revision == 5:
            ...
            self.O = computeO(
                self.userPassword, self.ownerPassword, self.revision
            )
            ...
            if DEBUG:
                print("self.key (hex)  = %s" % hexText(self.key))
            self.U = computeU(
                self.key, revision=self.revision, documentId=internalID
            )
            if DEBUG:
...
class StandardEncryptionDictionary(PDFObject):
    ...
    def __init__(self, O, OE, U, UE, P, Perms, revision):
        self.O, self.OE, self.U, self.UE, self.P, self.Perms = (
            O,
            OE,
            U,
            UE,
    ...
    if DEBUG:
        print(
            ...
            % tuple(
                [
                    hexText(str(x))
                    for x in (userPassword, ownerPassword, revision, O)
                ]
            )
        )
    return O
...
def computeU(
    encryptionkey, encodebytes=PadString, revision=None, documentId=None
):
    revision = checkRevision(revision)
    from reportlab.lib.arciv import ArcIV
    ...
    if revision == 2:
        ...
        assert (
            documentId is not None
        ), "Revision 3 algorithm needs the document ID!"
        h = md5(PadString)
...
def encodePDF(key, objectNumber, generationNumber, string, revision=None):
    ...
    if revision in (2, 3):
        ...
        padding = ""
        padding_len = (
            (16 - (string_len % 16)) if string_len > 16 else (16 - string_len)
        )
        if padding_len > 0:
...
def test():
    ...
    expectedO = (
        "<FA7F558FACF8205D25A7F1ABFA02629F707AE7B0211A2BB26F5DF4C30F684301>"
    )
    expectedU = (
        "<09F26CF46190AF8F93B304AD50C16B615DC43C228C9B2D2EA34951A80617B2B1>"
    )
            ...
            if thisarg[0] in argv:
                ...
                try:
                    if argv[pos + 1] not in known_modes:
                        ...
                        if verbose:
                            print(
                                "%s set to: '%s'." % (thisarg[3], argv[pos + 1])
                            )
                        argv.remove(argv[pos + 1])
