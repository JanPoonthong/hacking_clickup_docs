...
class FontDescriptor:
    ...
    def __init__(self):
        ...
        self.isItalic = (
            False  # true if it's italic or oblique or somehow slanty
        )
        ...
        self.metricsFileName = (
            None  # defined only for type='type1pc', or 'type1mac'
        )
...
class FontFinder:
    ...
    def getFamilyXmlReport(self):
        ...
        for dirName in self._dirs:
            lines.append(
                "    <directory name=%s/>" % quoteattr(asNative(dirName))
            )
        for familyName in self.getFamilyNames():
            if familyName:  # skip null case
                lines.append(
                    "    <family name=%s>" % quoteattr(asNative(familyName))
                )
