...
def setOutDir(name):
    ...
    if D:
        ...
        assert (
            name == "__main__"
        ), "setOutDir should only be called in the main script"
...
class GlobDirectoryWalker:
    ...
    def __init__(self, directory, pattern="*"):
        ...
        if os.path.isdir(directory):
            ...
            if (
                not isCompactDistro()
                or not __rl_loader__
                or not rl_isdir(directory)
            ):
                raise ValueError('"%s" is not a directory' % directory)
            self.directory = directory[
                len(__rl_loader__.archive) + len(os.sep) :
            ]
...
class ScriptThatMakesFileTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.chdir(scriptDir)
        assert os.path.isfile(self.scriptName), (
            "Script %s not found!" % self.scriptName
        )
...
def equalStrings(a, b, enc="utf8"):
    return (
        a == b if type(a) == type(b) else asUnicode(a, enc) == asUnicode(b, enc)
    )
