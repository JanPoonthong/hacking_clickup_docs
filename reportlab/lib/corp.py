...
class RL_CorpLogo(Widget):
    ...
    _attrMap = AttrMap(
        ...
        width=AttrMapValue(
            isNumber, desc="width in points of the logo (default 129)"
        ),
        height=AttrMapValue(
            isNumber, desc="height in points of the logo (default 86)"
        ),
    ...
    def draw(self):
        ...
        if bg:
            if shadow is not None and 0 <= shadow < 1:
                shadow = Color(
                    bg.red * shadow, bg.green * shadow, bg.blue * shadow
                )
        ...
        if bg or (bd and bdw):
            G.insert(
                0,
                Rect(
                    0, 0, _w, _h, fillColor=bg, strokeColor=bd, strokeWidth=bdw
                ),
            )
        if w != _w or h != _h:
...
class RL_CorpLogoThin(Widget):
    ...
    def draw(self):
        ...
        strokeColor = self.strokeColor
        shadow = Color(
            fillColor.red * sx, fillColor.green * sx, fillColor.blue * sx
        )
        g = Group()
        g2 = Group()
        g.add(
...
class ReportLabLogo:
    ...
    def __init__(
        self, atx=0, aty=0, width=2.5 * inch, height=1.5 * inch, powered_by=0
    ):
        self.origin = (atx, aty)
