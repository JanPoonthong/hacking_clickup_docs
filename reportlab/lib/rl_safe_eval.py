...
class UntrustedAstTransformer(ast.NodeTransformer):
    ...
    def visit_AugAssign(self, node):
        ...
        if isinstance(node.target, ast.Attribute):
            ...
            raise NotImplementedError(
                "Unknown target type: %s" % type(node.target)
            )
    ...
    def not_allowed(self, node):
        self.error(
            node, "%s statements are not allowed." % node.__class__.__name__
        )
...
def astFormat(node):
    return ast.dump(
        copy.deepcopy(node), annotate_fields=True, include_attributes=True
    )
...
class __RL_SAFE_ENV__(object):
    ...
    def __init__(self, timeout=None, allowed_magic_methods=None):
        ...
        ) + (() if isPy3 else (("file", __rl_missing_func__("file")),))
		...
		"""
                    + (
                        "__build_class__"
                        if isPy3
                        else "basestring cmp long unichr unicode xrange StandardError reduce apply"
                    )
                ).split()
            }
        )
        ...
        __rl_builtins__[
            "__rl_apply__"
        ] = lambda func, *args, **kwds: self.__rl_apply__(func, args, kwds)
    ...
    def __rl_unpack_sequence__(self, it, spec, _getiter_):
        ...
        for (idx, child_spec) in spec["childs"]:
            ret[idx] = self.__rl_unpack_sequence__(
                ret[idx], child_spec, _getiter_
            )
        return ret
    ...
    def __rl_apply__(self, func, args, kwds):
        ...
        return func(
            *[a for a in self.__rl_getiter__(args)],
...
class __rl_safe_eval__(object):
    ...
    def __call__(
        self, expr, g=None, l=None, timeout=None, allowed_magic_methods=None
    ):
        if not self.env:
