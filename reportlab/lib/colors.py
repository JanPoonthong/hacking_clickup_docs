...
class PCMYKColor(CMYKColor):
    ...
    def __repr__(self):
        return "%s(%s%s%s%s%s)" % (
            ...
            (
                self.density != 1
                and (",density=" + fp_str(self.density * 100))
                or ""
            ),
            (
                self.knockout is not None
...
def linearlyInterpolatedColor(c0, c1, x0, x1, x):
    ...
    if cname == "Color":  # RGB
        ...
        if cmykDistance(c0, c1) < 1e-8:
            ...
            assert (
                c0.spotName == c1.spotName
            ), "Identical cmyk, but different spotName"
            c = c0.cyan
            m = c0.magenta
            y = c0.yellow
            k = c0.black
            ...
            a = c0.alpha + x * (c1.alpha - c0.alpha) / dx
            return CMYKColor(
                c, m, y, k, density=d, spotName=c0.spotName, alpha=a
            )
            ...
            a = x * c1.alpha / dx
            return CMYKColor(
                c, m, y, k, density=d, spotName=c1.spotName, alpha=a
            )
...
class cssParse:
    ...
    def rgbVal(self, v):
        ...
        try:
            ...
            raise ValueError(
                "bad argument value %r in css color %r" % (v, self.s)
            )
    ...
    def hueVal(self, v):
        ...
        try:
            ...
            raise ValueError(
                "bad hue argument value %r in css color %r" % (v, self.s)
            )
    ...
    def __call__(self, s):
        ...
        if cmyk:
            ...
            if hsl:
                R, G, B = hsl2rgb(
                    self.hueVal(n[0]), self.pcVal(n[1]), self.pcVal(n[2])
                )
            else:
                R, G, B = list(
...
def parseColorClassFromString(arg):
    ...
    splitted = (
        "".join(arg.split()).replace("(", ",").replace(")", "").split(",")
    )
...
class toColor:
    ...
    def __call__(self, arg, default=None):
        ...
        if isinstance(arg, (tuple, list)):
            assert (
                3 <= len(arg) <= 4
            ), "Can only convert 3 and 4 sequences to color"
...
def _enforceSEP_BLACK(c):
    ...
    if not isinstance(tc, CMYKColorSep):
        if (
            ...
            isinstance(tc, CMYKColor)
            and tc.cyan == tc.magenta == tc.yellow == 0
...
def _chooseEnforceColorSpace(enforceColorSpace):
    if enforceColorSpace is not None and not isinstance(
        enforceColorSpace, collections.Callable
    ):
